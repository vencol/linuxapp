 ...
 * - 其他      读取超时或错误
 */
static int UART_Recv(int fd, char *rcv_buf, int data_len, int timeout)
{
    int len, fs_sel;
    fd_set fs_read;
    struct timeval time;
 ...
 * - -1            失败
 */
static int UART_Send(int fd, char *send_buf, int data_len)
{
    ssize_t ret = 0;
...
int main (int argc, char *argv[])
{
    ...
    while(1)    //循环读取数据
    {
        len = UART_Recv(fdSerial, rcv_buf, 99, 1000);
        if(len > 0)
        {
            rcv_buf[len] = '\0';
            printf("[%s] receive len:%d [%s] \n", DEV_NAME, len, rcv_buf);
            rcv_buf[len-2]++;
            UART_Send(fdSerial, rcv_buf, len);
        }
        else
        {
            UART_Send(fdSerial, "my test0", 9);//self send and self receive
